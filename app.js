...
app.get('/lambada', function (req, res) {
  res.status(200).json({ name: 'hienhhcc' });
});
...
app.get('/api/users', function (req, res) {
  return res.json({ name: 'ho hieu', email: 'hieu1@gmail.com' });
});
...
app.post('/api/users/register', function (req, res) {
  if (!req.body.email) {
    return res
      .status(400)
      .json({ message: 'The request is missing a required parameter `email`' });
  }
  if (!req.body.first_name) {
    return res
      .status(400)
      .json({
        message: 'The request is missing a required parameter `first_name`',
      });
  }
  if (!req.body.last_name) {
    return res
      .status(400)
      .json({
        message: 'The request is missing a required parameter `last_name`',
      });
  }
...
app.put('/api/users/:userid/roles', function (req, res) {
  return res
    .status(200)
    .json({ name: 'ho hieu', email: 'hieu1@gmail.com', role: 'user' });
});
